...
export async function getWork(
    ...
    if (debug) {
        console.log(
            `[assignments] name=${name} backlogDir=${ctx.backlogDir} entries=${output.entries.length} tasks=${output.tasks.length}`,
        );
        for (const e of output.entries) {
            const match = assigneeMatches(e.assignee);
            if (match || e.assignee) {
                console.log(
                    `[assignments] entry ${e.slug} assignee=${e.assignee ?? "(none)"} match=${match} status=${e.status}`,
                );
            }
        }
    }
    ...
    for (const entry of output.entries) {
        ...
        const allTasksDone =
            tasksForItem.length > 0 && tasksForItem.every((t) => t.status === "done");
        if (allTasksDone) {
