...
export function EditItemContentModal({
  itemSlug,
  onClose,
  onSaved,
}: {
  ...
  useEffect(() => {
    setError(null);
    const encoded = encodeURIComponent(itemSlug);
    fetch(`/api/items/${encoded}/content`)
      .then((res) => {
  ...
  return (
    <div
      ...
      data-testid="edit-content-modal-backdrop"
    >
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-4 flex flex-col max-h-[85vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <h3 className="text-lg font-semibold text-slate-800">Edit work item (index.md)</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="px-6 py-4 flex-1 min-h-0 flex flex-col">
          {loading ? (
            ...
            <>
              <label htmlFor="item-content" className="block text-sm font-medium text-slate-600 mb-2">
                Full index.md — METADATA (work, status, assignee) and body (description, context)
              </label>
              <textarea
                id="item-content"
                value={content}
