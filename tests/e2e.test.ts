...
function scaffoldBacklog(dir: string): void {
    ...
    fs.writeFileSync(path.join(itemDir, "001-setup.md"), taskFile("Setup", "open", "001"));

    fs.writeFileSync(path.join(itemDir, "002-implement.md"), taskFile("Implement", "open", "002"));
}
...
describe("e2e: write → dashboard → update → column movement", () => {
    ...
    it("completing all tasks moves item to Done column", async () => {
        let changeset = doc.changeTaskStatus("work/001-feat-dashboard-flow/001-setup.md", "done");
        await doc.commit(changeset);

        changeset = doc.changeTaskStatus("work/001-feat-dashboard-flow/002-implement.md", "done");
        await doc.commit(changeset);
    ...
    it("full lifecycle: open → in-progress → done in sequence", async () => {
        ...
        changeset = doc.changeTaskStatus("work/001-feat-dashboard-flow/001-setup.md", "done");
        await doc.commit(changeset);
